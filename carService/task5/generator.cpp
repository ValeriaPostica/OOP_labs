...
int main(int argc, char** argv) {
    ...
    try {
        ...
        // Fallback to current working directory if canonicalization fails
        exeDir = std::filesystem::current_path();
    }
    std::filesystem::path outputDir = exeDir / OUTPUT_DIR;
    ...
    for (int i = 1; i <= NR_OF_GENERATIONS; ++i) {
        ...
        // Write JSON to file
        std::string file_path = (outputDir / ("Car" + std::to_string(i) + ".json")).string();
    ...
    // Write finish.json with summary statistics for consumers to detect completion
    try {
        std::string finish_path = (outputDir / "finish.json").string();
        std::ofstream finish_file(finish_path);
        finish_file << stats.dump(4);
        finish_file.close();
    } catch (...) {
        // Swallow errors silently to avoid interrupting generator completion
    }
